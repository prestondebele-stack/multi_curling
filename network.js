...
const CurlingNetwork = (() => {
    ...
    function handleMessage(event) {
        ...
        switch (data.type) {
            ...
            case 'game_history':
                if (callbacks.onGameHistory) callbacks.onGameHistory({ games: data.games || [] });
                break;
            ...
            case 'game_forfeited':
                ...
                break;
            case 'game_cancelled':
                if (callbacks.onGameCancelled) callbacks.onGameCancelled(data);
                break;
            case 'invite_declined':
                if (callbacks.onInviteDeclined) callbacks.onInviteDeclined(data);
                break;
    ...
    return {
        ...
        sendCancelGame(code) { send({ type: 'cancel_game', code }); },
        sendDeclineInvite(code) { send({ type: 'decline_invite', code }); }, // v127b
        ...
        onGameHistory(cb) { callbacks.onGameHistory = cb; },
